...
- enforce-boundaries.sh — PreToolUse Write/Edit. Blocks outside `$CLAUDE_PROJECT_DIR` (harness-state exception `~/.claude/projects/{slug}/`). Blocks sensitive files (.env, *.pem, *.key, SSH, credentials.json, certs, .keystore). Cascades to git-agent.

 ...
 *
 * Flow: Generate a single repo health report (flow-1)
 * Read-only flow — repo-health-class. Submit a repo URL, see charts.
...
test.describe("Generate a single repo health report (flow-1)", () => {
  test("walks 5 interaction(s) deterministically", async ({ page }) => {
    ...
    try {
      ...
      await page.waitForResponse(
        (r) => new RegExp("/api/report/").test(r.url()) && r.status() === 200,
      );
      __stepIndex = 5;
      await expect(
        page.locator('[data-kit-component="ChartFrame"]'),
      ).toBeVisible();
    } catch (err) {
      ...
      throw new Error(
        `flow-1 (Generate a single repo health report) failed at interaction ${__stepIndex}: ${message}`,
      );
    }
  });
});

...
import { forecast, classifyForecast, PRICING } from "../src/cost-projection.js";
...
describe("cost-projection.classifyForecast", () => {
  it("under 50% → ok", () => {
    expect(classifyForecast({ forecastUsd: 0.49, perStageCapUsd: 1.0 })).toBe(
      "ok",
    );
  });
  it("at 50% → warn", () => {
    expect(classifyForecast({ forecastUsd: 0.5, perStageCapUsd: 1.0 })).toBe(
      "warn",
    );
  });
  it("above 50%, below 100% → warn", () => {
    expect(classifyForecast({ forecastUsd: 0.99, perStageCapUsd: 1.0 })).toBe(
      "warn",
    );
  });
  it("at cap → throw", () => {
    expect(classifyForecast({ forecastUsd: 1.0, perStageCapUsd: 1.0 })).toBe(
      "throw",
    );
  });
  it("over cap → throw", () => {
    expect(classifyForecast({ forecastUsd: 1.5, perStageCapUsd: 1.0 })).toBe(
      "throw",
    );
  });
});
...
describe("cost-projection.PRICING — May 2026 baseline assertions", () => {
  ...
  it("cache read = 10% of base input price per tier", () => {
    for (const t of ["haiku", "sonnet", "opus"] as const) {
      expect(PRICING[t].cacheReadPerM).toBeCloseTo(
        PRICING[t].inputPerM * 0.1,
        6,
      );
    }
  });
  it("cache write 5m = 1.25× base input price; 1h = 2× base", () => {
    for (const t of ["haiku", "sonnet", "opus"] as const) {
      expect(PRICING[t].cacheWrite5mPerM).toBeCloseTo(
        PRICING[t].inputPerM * 1.25,
        6,
      );
      expect(PRICING[t].cacheWrite1hPerM).toBeCloseTo(
        PRICING[t].inputPerM * 2.0,
        6,
      );
    }
  });

...
const FIXTURES = [
  ...
  // block-dangerous benign
  {
    ...
    tool: "Bash",
    command: "git push --force-with-lease origin feat/x",
    expected: "allow",
  },
  {
    hook: "block-dangerous",
  ...
  {
    hook: "block-dangerous",
    name: "force-push-feature-branch",
    tool: "Bash",
...
const HOOK_PATHS = {
  "block-dangerous": [
    "bash",
    path.join(ROOT, ".claude/hooks/block-dangerous.sh"),
  ],
  "enforce-boundaries": [
    "bash",
    path.join(ROOT, ".claude/hooks/enforce-boundaries.sh"),
  ],
  "detect-loop": ["node", path.join(ROOT, ".claude/hooks/detect-loop.mjs")],
  "validate-brief": [
    "node",
    path.join(ROOT, ".claude/hooks/validate-brief.mjs"),
  ],
...
if (jsonOut) {
  console.log(
    JSON.stringify({ total: results.length, pass, fail, results }, null, 2),
  );
} else {
  ...
  for (const r of results) {
    const tag = r.ok ? "  ✓" : "  ✗";
    console.log(
      `${tag} [${r.fixture.hook}] ${r.fixture.name} — expected ${r.fixture.expected}, got ${r.verdict}`,
    );

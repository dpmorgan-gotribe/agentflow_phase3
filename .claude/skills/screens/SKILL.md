...
##### Canonical chrome reference — `docs/design-system-preview.html` (phase1-step-036 / bug-004)

The design-system-preview.html generated by `/stylesheet` and signed off at gate 3 is the canonical chrome reference for the project. Every screen's page-level chrome MUST match the preview's vocabulary; the operator approved THAT chrome, not an in-screen reinvention of it.

When assembling the shared preamble (step 3.5.1), additionally parse `docs/design-system-preview.html` and inline two consistency contracts derived from it:

1. **Canonical footer background class.** Find the page-level `<footer>` in the preview; extract the `bg-*` utility class on it (e.g. `bg-surface-base`, `bg-neutral-900`, `bg-secondary-500`). Inline this verbatim in the preamble under `### Canonical footer background`. EVERY screen's page-level `<footer>` MUST emit this exact class. Do NOT use a different bg class (e.g. don't pick `bg-surface-inverted` when the preview committed `bg-surface-base`). If the preview has no explicit bg-class, omit this contract — the audit won't fire.

2. **Canonical dark-band text vocabulary.** Find every block in the preview whose background resolves to "dark" (`bg-surface-inverted`, `bg-neutral-{800,900,950}`, `bg-secondary-{500,600}`, `bg-primary-{800,900}`, `bg-accent-{800,900}`, `bg-black`). Collect the set of `text-*` utility classes used as descendants inside those blocks (e.g. `text-text-inverted`, `text-text-inverted/85`, `text-white/70`). Inline this set verbatim in the preamble under `### Canonical dark-band text vocabulary`. EVERY screen's dark-band block MUST only use text-color classes from this set. Do NOT mix in light-mode text tokens (`text-text-secondary`, `text-text-tertiary`) — they're mid-grey on near-black and fail WCAG AA contrast even though their class name suggests "secondary text".

Both contracts are project-agnostic: a project whose preview committed a black footer with low-contrast typography passes the audit if every screen mirrors that vocabulary; a project whose preview committed a warm-paper footer with high-contrast typography passes if every screen mirrors THAT. What the audit enforces is consistency-within-project, derived from the preview the operator signed off on.
...
- `docs/design-system-preview.html` → canonical footer-bg class + canonical dark-band text-color vocabulary (phase1-step-036 / bug-004)

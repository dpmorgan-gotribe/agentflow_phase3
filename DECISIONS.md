...
### Revision 2026-05-28 — skill-body auto-run wiring (the implementation that was missing)

The original 2026-05-28 ADR-005 added the `userInvokable: boolean` field to stages-array but only at the metadata/documentation layer. The 4 parent skill bodies (`.claude/skills/{analyze,screens,architect,pm}/SKILL.md`) were not updated to actually invoke the child stages — they retained the Phase 2 assumption that the cli-runner orchestrator owns the sequencing. The screens skill even had an explicit "Skill does NOT auto-invoke" acceptance criterion line that contradicted the new ADR.

This gap surfaced when the operator ran `/screens` in manual operator mode (not via cli-runner). The skill did the primary work but did NOT auto-invoke `/visual-review` or `/user-flows-generator` — the operator was asked to run them manually, defeating ADR-005's "6 operator-invokable commands" UX promise.

**Implementation added:** each of the 4 parent SKILL.md bodies now contains a `## Auto-run chain (ADR-005)` section that (1) names the child(ren) to invoke via the Skill tool in order; (2) specifies success/failure handling per child; (3) documents the idempotency contract — each child checks "do my outputs exist with current fingerprint?" and no-ops if yes, letting the chain run unconditionally without needing to detect pipeline-vs-manual mode; (4) for `/analyze` and `/architect`, documents the gate-wait behavior (Gate 1 for analyze, Gate 5 for architect fire BEFORE the child runs; manual-mode polling for the gate file is described). The screens skill's prior "Skill does NOT auto-invoke" line replaced with the inverse.

Lesson captured: LESSONS.md "ADR-005 metadata is not implementation" (phase1-step-015 area). Decisions documented in a planning artifact don't ship unless the implementation layer is updated to honor them — the test is whether an operator running the affected command sees the documented behavior end-to-end.
